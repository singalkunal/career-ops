...
export function ApplyProvider({ children }: { children: React.ReactNode }) {
  ...
  const fill = useCallback(async () => {
    ...
    setSteps([]);
    try {
      const r = await fetch("/api/apply/fill", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: sessionId.current, answers, fields, handoff: true, company: companyRef.current, n: nRef.current }) });
      const d = await r.json();
